$(document).ready(function () {
	...
	var email_valid = /(^[a-zA-Z0-9._]+@([a-zA-Z0-9]+\.[a-zA-Z0-9]+)+\s?$)/g;
	var cpf_valid = /(^[0-9]{11}\s?$)|(^([0-9]{3}\.){2}[0-9]{3}\-[0-9]{2}\s?$)/g;
	var rg_valid = /(^[0-9]{6,14}\s?$)|(^([0-9]|[0-9][0-9\-\.]){6,14}\s?$)/g;
	var cep_valid = /^(4[0-8]([0-9]){6}\s?)|(4[0-8]([0-9]){3}\-[0-9]{3}\s?)$/g;
	var uf_valid = /^[a-zA-Z]{2}\s?$/g;
